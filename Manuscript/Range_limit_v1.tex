...
\end{aligned}
\end{equation}
\noindent Here, $P_i$ is total precipitation (mm) cumulated over the 12 months preceding the census, $endo_i$ indicates endophyte presence, and $herb_i$ indicates herbivore access.
Random effects account for background variation at multiple hierarchical levels: $\phi_{\text{site}[i],Sp_i}$ captures site-to-site heterogeneity, $\omega_{\text{source}[i],Sp_i}$ captures variation associated with the provenance of transplants, and $\rho_{\text{plot}[i],Sp_i}$ captures plot-to-plot variation.
...
Secondly, we compared quadratic and monotonic candidate models using leave-one-out cross-validation (LOOCV) to identify the best-supported model for each vital rate \citep{vehtari2017practical}.
...
Finally, for each vital rate, precipitation was summarized at key quantiles (10th, 25th, 50th, 75th, and 90th percentiles) to represent the range of site-level abiotic stress. 
...
\subsection*{Drivers of endophyte effects on cool-season grass demography}
To identify the main drivers of endophyte-mediated effects, we fitted three hierarchical Bayesian candidate models for each vital rate (survival, growth, inflorescence, and spikelet production; Supplementary Methods \ref{sec:models}).
Each model included endophyte status as a baseline predictor and differed in whether it incorporated interactions with precipitation (Endophyte~$\times$~Climate), herbivory (Endophyte~$\times$~Herbivory), or both factors, including the three-way interaction (Endophyte~$\times$~Climate~$\times$~Herbivory).
Random effects for site-year ($\phi_{\text{site}[i],\text{Sp}_i}$), source population ($\omega_{\text{source}[i],\text{Sp}_i}$), and plot ($\rho_{\text{plot}[i],\text{Sp}_i}$) captured hierarchical variation while accommodating species-specific responses.
We compared candidate models using leave-one-out cross-validation (LOO-CV) \citep{vehtari2017practical}, which estimates predictive accuracy by iteratively holding out each observation. Differences in expected log predictive density ($\Delta$ELPD), along with associated standard errors, were used to rank model performance.
...
Across fitness components, \textit{Epichloë} endophytes generally enhanced host performance under wetter conditions, whereas their effects were costly or occasionally neutral under drier conditions, consistent with the inhibition scenario (Fig.~\ref{fig:prediction}). 
...
In \textit{Agrostis hyemalis} and \textit{Elymus virginicus}, median $\Delta(E^{+} - E^{-})$ ranged from approximately $-0.026$ to $0.019$ under low precipitation in unfenced plots ($\mathrm{Pr}(\Delta>0)=0.37$--0.61) and increased to $0.022$--$0.106$ under high precipitation ($\mathrm{Pr}(\Delta>0)=0.70$--0.85; Table~\ref{tab:delta_summary_surv}). 
...
\textit{Epichloë} endophyte presence generally enhanced relative growth of individuals across the precipitation gradient, with benefits increasing under wetter conditions and not modulated by herbivory (Table~\ref{tab:delta_summary_grow}).
...
In \textit{P.~autumnalis}, growth was context-dependent: unfenced plots shifted from negative to positive effects across the precipitation gradient (median $\Delta=-0.275$ to 0.144; $\mathrm{Pr}(\Delta>0)=0.169$--0.775), while fencing generally amplified benefits at low to moderate precipitation (median $\Delta=0.020$--0.390; $\mathrm{Pr}(\Delta>0)=0.559$--0.927), but became negative at higher precipitation (median $\Delta=-0.262$ to $-0.322$; $\mathrm{Pr}(\Delta>0)=0.056$--0.072).

\textit{Epichloë} endophytes consistently increased inflorescence production across the precipitation gradient (Fig.~\ref{fig:flowering}). 
...
Inflorescence benefits in \textit{A.~hyemalis} and \textit{E.~virginicus} were minimal under dry conditions but increased with precipitation, with only minor influence from herbivore exclusion. 
...
Across all vital rates (survival, growth, inflorescence, and spikelet production), models comparing different drivers of the endophyte--host interaction consistently identified the \textit{Endophyte~$\times$~Climate} model as the best predictor of host demography (Table~\ref{tab:comp_table_driver}). 
...
Endophyte presence often enhances host performance under favorable or moderately stressful conditions, a pattern widely documented across grass–\textit{Epichloë} systems \citep{clay1990fungal}.
...
In our field experiment, we observed higher growth and survival of endophyte-infected grasses in wetter sites, consistent with previously documented positive effects of endophytes under favorable conditions \citep{afkhami2014mutualist,rudgers2020climate}.
...
While endophyte infection enhanced host performance under favorable or moderately stressful conditions, its effects diminished and ultimately became costly under high abiotic stress, even in fenced plots where herbivores were excluded. 
...
Under low-precipitation conditions, reduced water availability constrains nutrient uptake and increases the relative metabolic burden of maintaining the endophyte, potentially tipping the cost–benefit balance against endophyte infected plants \citep{ahlholm2002vertically,bronstein2001exploitation,faeth2002endophytic}. 
...
Although we found no evidence of infection-related costs for inflorescences across the precipitation gradient, spikelet production showed a small but consistent cost at the driest sites. 
This indicates that drought-driven resource limitation disproportionately affects certain reproductive components in infected plants, with different traits varying in their sensitivity to shifts in host–endophyte interactions \citep{cheplick2009ecology}.

Across species and fitness components, climatic variation was the primary driver of endophyte-mediated effects, whereas herbivory played a secondary or context-dependent role. 
Precipitation strongly shaped host demography in our system, with drier sites constraining survival, growth and fertility. 
Contrary to the predictions of the stress gradient hypothesis (SGH), which suggests that biotic interactions should become more positive under stressful conditions, endophyte-mediated benefits were reduced under high abiotic stress \citep{louthan2015and,lortie2006re}. 
Although most empirical tests of the SGH have focused on plant–plant interactions and have been developed and tested mainly by community ecologists \citep{bertness1994positive}, recent syntheses show that support for the SGH is actually strongest among microbial interactions \citep{piccardi2019toxicity,hammarlund2019refining}, with bacterial systems showing greater conformity to SGH predictions than any other kingdom \citep{adams2022case}. 
This contrast highlights that plant–microbe mutualisms, including fungal endophytes, may not follow the same stress-dependency patterns documented in plant community ecology, and that endophyte benefits can decline rather than intensify under severe environmental stress.
...
When host resources are constrained, the costs of maintaining the symbiosis may outweigh its benefits, leading to reduced endophyte-mediated effects under high abiotic stress.
