...
\item Herbivory- climate-dependent effects: The effect of endophyte symbiosis depends on the joint influence of herbivory and precipitation, such that exposure to herbivory amplifies endophyte benefits under more arid conditions (Fig.~\ref{fig:prediction}d).
\end{enumerate}
...
\revise{\linelabel{genotypedef}We refer to each independently derived source individual (originating from a single seed) as a ``genotype'' and consider all of its vegetatively propagated descendants to belong to the same genotype, such that all clones share the same genetic background of the host and (for $S^+$) symbiont.}
...
\revise{\linelabel{nbind}Each plot was planted with 15 individuals, with 12, 9, 6, or 3 $S^+$ plants and the remainder $S^-$, corresponding to initial endophyte frequencies of 80\%, 60\%, 40\%, or 20\% $S^+$, respectively. This variation in initial endophyte prevalence was intended to track change in prevalence from different starting conditions and is not further considered here.}
...
\end{aligned}
\end{equation}
\noindent Here, $precip_i$ is realized annual precipitation for a given site and year, $symb_i$ indicates endophyte presence ($symb_i=1$) or absence ($symb_i=0$), and $herb_i$ indicates fencing treatment: herbivore access ($herb_i=0$) or exclusion ($herb_i=1$). }
...
Under herbivore exclusion, endophyte effects on \textit{P. autumnalis} growth shifted from weakly positive to negative with increasing precipitation ($\Delta = -0.22$, $\mathrm{Pr}(\Delta > 0) = 0.13$), the only species / vital rate pair that was even directionally consistent with drought-protective benefits of symbiosis.
...
However, the role of herbivory as a modifier of endophyte effects on  other vital rates were more idiosyncratic. 
...
\revise{The physiological mechanisms underlying the decline in endophyte benefits with increasing aridity are unclear, but several non-exclusive processes may contribute.} 
...
\revise{Our results are also anticipated by empirical patterns along precipitation gradients in other grass--\textit{Epichloë} systems \citep{kannadan2008endophyte}, and by a systematic review noting that positive endophyte effects on drought tolerance are inconsistent across species and understudied for fitness measures such as fecundity and survival \citep{decunta2021systematic}.}
