...
Yet, how endophytes affect host demography across abiotic stress gradients when hosts are subject to herbivory pressure remains unclear.
...
At each site, we established eight 1.5 m $\times$ 1.5 m plots, each containing 18 individuals of one of three cool-season perennial grasses: \emph{Agrostis hyemalis}, \emph{Elymus virginicus}, and \emph{Poa autumnalis}.  
These species typically host seed-transmitted fungal endophytes \citep{shaw2011guide}.  
...
\caption{Distribution of common garden sites along a natural aridity gradient from Texas to Louisiana in the United States for: 
...
\subsection*{Modeling the effects of aridity, herbivory  and endophyte on cool-season grass demography}
...
\end{aligned}
\end{equation}
\noindent where $P$ is precipitation, $endo$ indicates endophyte presence, and $herb$ indicates herbivory. 
...
We used Bayesian mixed-effect models to test the effects of abiotic stress (low vs. high precipitation), endophyte association (E$^{-}$ vs. E$^{+}$), and herbivore exclusion (fenced vs. unfenced) on survival, growth, and reproductive traits in three cool-season grass species: \textit{Agrostis hyemalis}, \textit{Elymus virginicus}, and \textit{Poa autumnalis}. 
All models effectively captured key aspects of the data, including means, standard deviations, and quantiles (Fig.~\ref{sup:ppc_vital}).  
High abiotic stress reduced survival, with E$^{-}$ individuals outperforming E$^{+}$ individuals, indicating a cost of endophyte association under dry conditions (Fig.~\ref{fig:Surv}).
...
\caption{Predicted relative growth (top panels) and endophyte effect (\(\Delta = \mathrm{E^+ - E^-}\), bottom panels) across precipitation gradients. 
Lines show posterior means for E\(^+\) (blue) and E\(^-\) (red), with shading indicating 90\% credible intervals. 
...
Across all vital rates, survival, growth, flowering, and spikelet production, models including only abiotic predictors consistently had the highest ELPD values (Figure~\ref{fig:bestdriver}). 
The standard errors of the ELPD differences indicate some uncertainty in these comparisons. 
For example, the difference between the abiotic-only and abiotic+biotic models for survival was small relative to its uncertainty ($\Delta \mathrm{ELPD} = -2.1$, SE = 3.7), suggesting that including biotic predictors provides only marginal additional information. 
In contrast, biotic-only models had substantially lower ELPD values with smaller uncertainty (e.g., survival: $\Delta \mathrm{ELPD} = -5.2$, SE = 2.0), indicating they are clearly inferior. 
Overall, these results suggest that the demography of \textit{Agrostis hyemalis}, \textit{Elymus virginicus}, and \textit{Poa autumnalis} is primarily driven by variation along the abiotic stress gradient, such as precipitation, whereas biotic interactions, including herbivory and endophyte presence, play a secondary role. 
This pattern aligns with the stress gradient hypothesis, which predicts that abiotic factors have a stronger influence on species performance in stressful environments, while biotic interactions become more important under more benign conditions.


\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{bestdriver.pdf}
...
Common gardens were established at seven sites spanning the geographic range of \emph{Elymus virginicus} from Louisiana to Texas, USA. Sites were selected along a pronounced aridity gradient, with Louisiana representing mesic conditions and Texas increasingly arid conditions. Mean annual temperature did not differ significantly among sites (Fig.~\ref{Sup:temp}), confirming that precipitation is the primary abiotic gradient. Sites were chosen to reflect natural environmental conditions, including shaded areas under tree canopy or near shrubs.
