...
data {
  ...
  vector[N] clim;
  vector[N] clim2; // quadratic climate term
}
...
parameters {
  ...
  real mu_b0;
  real mu_bclim;
  real mu_bclim2;          // quadratic climate
  real mu_bendo;
  real mu_bherb;
  real mu_bendoclim;
  real mu_bherbclim;
  ...
  real<lower=0> sigma_bclim2; // quadratic climate
  ...
  vector[nSpp] z_b0;
  vector[nSpp] z_bclim;
  vector[nSpp] z_bclim2;    // quadratic climate
  vector[nSpp] z_bendo;
  vector[nSpp] z_bherb;
...
transformed parameters {
  ...
  vector[nSpp] bclim2        = mu_bclim2        + sigma_bclim2        * z_bclim2; // quadratic climate
  ...
  for (i in 1:N) {
    predF[i] =
      b0[Spp[i]]
      + bclim[Spp[i]]         * clim[i]
      + bclim2[Spp[i]]        * clim2[i]       // quadratic climate
      + bendo[Spp[i]]         * endo[i]
      + bherb[Spp[i]]         * herb[i]
...
model {
  ...
  mu_b0            ~ normal(0, 2);
  mu_bclim         ~ normal(0, 2);
  mu_bclim2        ~ normal(0, 2); // quadratic climate
  mu_bendo         ~ normal(0, 2);
  mu_bherb         ~ normal(0, 2);
  ...
  sigma_bclim2        ~ normal(0, 1); // quadratic climate
  ...
  z_b0            ~ normal(0, 1);
  z_bclim         ~ normal(0, 1);
  z_bclim2        ~ normal(0, 1); // quadratic climate
  z_bendo         ~ normal(0, 1);
  z_bherb         ~ normal(0, 1);
