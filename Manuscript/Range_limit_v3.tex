...
\subsection*{Abiotic and biotic factors jointly shape fitness components across the stress gradient}
...
Inflorescence production was best predicted by the \textit{Endophyte}~$\times$~\textit{Herbivory} model, with modest differences from the \textit{Endophyte}~$\times$~\textit{Herbivory} model ($\Delta \mathrm{ELPD} = -1.3$, SE = 2.3) and the \textit{Endophyte}~$\times$~\textit{Climate} model ($\Delta \mathrm{ELPD} = -1.4$, SE = 2.8). 
Similarly, spikelet production was best supported by the \textit{Endophyte}~$\times$~\textit{Herbivory} model, whereas the abiotic–biotic interaction model showed lower predictive performance ($\Delta \mathrm{ELPD} = -4.2$, SE = 1.7).
\section*{Discussion}
