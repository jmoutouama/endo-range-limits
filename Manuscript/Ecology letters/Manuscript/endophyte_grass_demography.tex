...
Species differences suggest that the decline in endophyte benefits with aridity is not uniform across host--symbiont combinations. \textit{P. autumnalis} maintained strong reproductive benefits regardless of climate, indicating relative buffering against abiotic stress, but weaker endophyte effects on survival and growth. 
...
Endophytes reduced herbivory damage in \textit{A. hyemalis} and \textit{P. autumnalis}, consistent with alkaloid-mediated defense, but increased damage in \textit{E. virginicus} (Fig.~\ref{sup:herbivory}), possibly because endophyte-symbiotic plants were more vigorous and therefore more attractive to herbivores. 
...
These results suggest that differences in soil microbial composition, nutrient availability, or water retention are unlikely to be the primary drivers of the field patterns, although edaphic effects cannot be fully excluded given the inferential nature of the experiment. 
...
Our findings suggest that the degree of host control over symbiont retention is an underappreciated axis shaping mutualism outcomes along environmental gradients \citep{bronstein2001exploitation}. 
Because \textit{Epichloë} endophytes are tightly integrated with host reproduction and are not readily shed, hosts may continue to bear symbiont-associated costs even when environmental conditions reduce net benefits. 
Under such conditions, positive effects may emerge only when resource availability is sufficient to offset these costs. 
Our results therefore suggest that the stress-gradient hypothesis may require additional nuance when applied to symbioses in which hosts have limited flexibility to regulate symbiont load under stress \citep{david2018microbial}. 
...
Our results provide a potential demographic mechanism for these patterns: as aridity intensifies, vertically transmitted symbionts may shift from beneficial to costly precisely where hosts are most vulnerable. 
Integrating host control over symbiont persistence and host physiological condition into predictions of symbiosis outcomes under climate change may therefore improve forecasts of whether grass--endophyte associations buffer or amplify demographic declines at range margins.
