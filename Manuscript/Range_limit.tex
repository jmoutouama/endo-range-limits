...
We studied three cool-season grasses hosting seed-transmitted fungal endophytes using geographically distributed field experiments along a natural aridity gradient, with factorial manipulations of endophyte presence and herbivory. 
Bayesian models showed that  all components of host fitness (survival, growth and fertility) declined toward more arid sites, indicating reduced performance under stress. 
...
Herbivory had no detectable effect on demography. 
 Abiotic stress was the primary driver of vital rates, with herbivory and endophytes having little effect. 
...
Uncertainty estimates showed that in some cases the advantage of abiotic-only models over those that also included biotic predictors was small. For example, for survival, the abiotic-only model outperformed the abiotic and biotic model only slightly 
...
is shaped primarily by abiotic stressors such as precipitation, while biotic interactions (herbivory and endophyte presence) play a secondary role. 
This pattern aligns with the stress gradient hypothesis, which predicts that abiotic factors dominate in stressful environments, whereas biotic interactions gain importance under more benign conditions.
