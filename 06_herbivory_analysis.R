...
init_plot <- left_join(init23,plots %>% dplyr::select(Site,Plot,Species,Herbivory),
...
init_combo<-bind_rows(init_plot,init24 %>% dplyr::select(names(init_plot))) %>% 
  filter(!is.na(Tag_ID))
...
bind_rows(dat23 %>% 
            mutate(Year=2023,
                   AttachedInf=NA,
                   BrokenInf=NA) %>% 
            dplyr::select(Year,Site,Species,Plot,Herbivory,Tag_ID,Population,Endo,Tiller,AttachedInf,BrokenInf,TillerHerb),
          dat24 %>% 
            mutate(Year=2024) %>% 
            dplyr::select(Year,Site,Species,Plot,Herbivory,Tag_ID,Population,Endo,Tiller,AttachedInf,BrokenInf,TillerHerb),
          dat25 %>% 
...
herb_dat<-herb_dat_tidy %>% 
  ...
  filter(Tiller>0) %>% 
  dplyr::select(Species,Site,Plot,Endo,Herbivory,Year,Tiller,TillerHerb) %>% 
  drop_na() %>% 
...
herbendo<-ggplot(beta_summary,
  ...
  scale_y_discrete(
    labels = c(
      "AGHY" = expression(italic("Agrostis hyemalis")),
