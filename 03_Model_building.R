...
demography_surv_ppt <- list(
  ...
  clim2      = as.vector(demography_climate_surv$ppt^2),  
...
demography_grow_ppt <- list(
  ...
  clim2= as.vector(demography_climate_grow$ppt^2),  
...
demography_inf_ppt <- list(
  ...
  clim2 = as.vector(demography_climate_inf$ppt^2),
...
demography_spik_ppt <- list(
  ...
  clim2 = as.vector(demography_climate_spik$ppt^2),
...
combined_plot <- (p_surv | p_grow) / (p_inf | p_spik) +
...
# Save to PDF
ggsave(filename = "/Users/jacobmoutouama/Dropbox/Miller Lab/range limits model output/combined_ppc_plots_quadratic.pdf", plot = combined_plot, width = 7, height = 6)
