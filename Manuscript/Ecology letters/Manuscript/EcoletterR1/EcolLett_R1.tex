...
\revise{\linelabel{inflorescence}Hurdle and zero-inflated negative binomial models had nearly identical predictive performance ($\Delta$ELPD = 0.2, SE = 3.1). We retained the hurdle model because it distinguishes whether a plant flowered and, if so, how many inflorescences it produced. Its two components, a Bernoulli model for flowering and a zero-truncated negative binomial model for inflorescence count, both used the linear predictor in Eq.~\ref{eq:full_model}.}
...
Quadratic effects of precipitation provided minimal improvement over linear effects (LOOCV; Table~\ref{tab:model_comparison}), so we used the linear model, in which the expected value for observation $i$ ($\mu_{i}$) was:
...
\revise{Endophyte\linelabel{herbivoryresults} symbiosis reduced the predicted proportion of damaged tillers in \textit{P. autumnalis} ($S^+$: 13.5\% vs.\ $S^-$: 17.1\%) and, more weakly, in \textit{A. hyemalis} ($S^+$: 12.8\% vs.\ $S^-$: 14.2\%), whereas it increased damage in \textit{E. virginicus} ($S^+$: 62.8\% vs.\ $S^-$: 59.4\%; Fig.~\ref{sup:herbivory}).}
...
Points indicate observed plot means, with point size proportional to sample size.
Horizontal dashed line denotes $\Delta = 0$. 
Lines curve because growth was modelled as a linear function of log(precipitation).
Panels are arranged by species and herbivory treatment (herbivore access and exclusion).}
\label{fig:growth}
\end{figure}
\clearpage
