...
ppt_mean <- mean(climate_scaled$ppt_log, na.rm = TRUE)
ppt_sd   <- sd(climate_scaled$ppt_log, na.rm = TRUE)

# Sanity check: these MUST match the mean/sd of ppt_log that were used to
# build ppt_scaled back in 03_Model_building.R, or every climate_mm / clim_mm
# back-transform in this script (survival, growth, inflorescence, spikelet)
# will be silently wrong (or NA, which drops points from the plots).
stopifnot(!is.na(ppt_mean), !is.na(ppt_sd))
if (any(is.na(climate_scaled$ppt_log))) {
  message("Note: climate_scaled$ppt_log contains NAs (excluded via na.rm = TRUE).")
}
...
grow_xmin_by_species <- plot_data_grow %>%
  filter(panel == "Growth") %>%
  group_by(species) %>%
  summarise(xmin = min(climate_mm, na.rm = TRUE), .groups = "drop")

panel_labels_grow <- panel_labels_grow %>%
  left_join(grow_xmin_by_species, by = "species")
...
ggplot(plot_data_grow) +
  ...
  # ── NEW: log x-axis so the linear-in-log-precip effect renders as a straight line ──
  scale_x_continuous(
    trans  = "log10",
  ...
  # ── CHANGED: anchor to panel corner (-Inf) instead of a fixed mm value,
  #             since a fixed x = 490 no longer sits in the same place under log-x ──
  geom_text(
    data = panel_labels_grow,
    aes(x = xmin, y = ymax * 0.70, label = label),
    hjust = -0.2, size = 4.7, inherit.aes = FALSE
  )
dev.off()
