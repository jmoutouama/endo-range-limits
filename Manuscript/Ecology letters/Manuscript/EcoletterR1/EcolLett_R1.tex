...
\bigskip
\noindent\textbf{This file contains:} Abstract (150 words), Main Text (4998 words), Figures (1-6), Supporting Information - Supplemental Methods, 
Supplemental Figures S1-S12, References (75).
...
Seeds from control and heat-treated lineages were surface-sterilized with a  \revise{\linelabel{beach}10\% bleach solution \citep{Sahu2022sterilization}.
...
All models included species-specific fixed effects for symbiosis ($S^+$ or \revise{\linelabel{Sminus7}$S^-$}), site- and year-specific precipitation, and herbivory (herbivore access/exclusion), as well as all two- and three-way interactions among them.
...
\end{aligned}
\end{equation}
\noindent Here, $precip_i$ is cumulative precipitation between transplanting and the corresponding census for a given site and year, $symb_i$ indicates endophyte presence ($symb_i=1$) or absence ($symb_i=0$), and $herb_i$ indicates fencing treatment: herbivore access ($herb_i=0$) or exclusion ($herb_i=1$). }
...
Endophytes reduced herbivore damage in \textit{A. hyemalis} and \textit{P. autumnalis}, in keeping with alkaloid-mediated defense, but increased damage in \textit{E. virginicus} (Fig.~\ref{sup:herbivory}), possibly because symbiotic plants were more vigorous and therefore more attractive to herbivores. \revise{\linelabel{alkaloid} 
...
Precipitation was log-transformed for analysis and is shown on a logarithmic axis, so equal distances represent equal proportional changes (each labelled tick is double the previous one).
