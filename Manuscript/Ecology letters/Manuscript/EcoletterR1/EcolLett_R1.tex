...
Seeds from these source populations (collected from ca. 30 individuals per population) were either heat-treated to remove endophytes or left untreated (control), intended to produce lineages of symbiont-free (\revise{\linelabel{Sminus1}$S^-$}) and symbiotic ($S^+$) plants from the same genetic background. 
...
For all but two populations, the \revise{\linelabel{Sminus2}$S^-$}\ plants used in our experiment were reared through one or more generations following heat treatment, which minimized the confounding potential of direct physiological effects of heating seeds (Table \ref{tab:heat_treatment}). 
...
The experiment included 840, 840, and 720 plants of \textit{A. hyemalis}, \textit{E. virginicus}, and \textit{P. autumnalis}, respectively, with half assigned to each endophyte status ($S^+$ and \revise{\linelabel{Sminus3}$S^-$}), from three, four, and four source populations, respectively.
...
We established common garden experiments with $S^+$ and \revise{\linelabel{Sminus4}$S^-$} hosts at seven sites along an aridity gradient from Louisiana to central Texas, US (Fig.~\ref{fig:site}, Table~\ref{tab:exp_sites}).
...
\revise{\linelabel{nbind}Each plot was planted with 15 individuals, with 12, 9, 6, or 3 $S^+$ plants and the remaining individuals being $S^-$, corresponding to starting endophyte frequencies of 80\%, 60\%, 40\%, or 20\% $S^+$, respectively. This variation in initial endophyte prevalence was not further considered here.}
...
For each species at each site, we planted a total of 60 $S^+$ and 60 \revise{\linelabel{Sminus5}$S^-$} plants, half of those experiencing herbivore access and half experiencing herbivore exclusion. 
...
All models included species-specific fixed effects for symbiosis ($S^+$ or \revise{\linelabel{Sminus7}$S^-$}), site- and year-specific precipitation (cumulative rainfall [mm] over 12 months preceding the census), and herbivory (herbivore access/exclusion), including all two- and three-way interactions. 
...
For each herbivory treatment (Access or Exclusion), we computed posterior predictions for the vital rates of $S^+$ and \revise{\linelabel{Sminus8}$S^-$} hosts across the precipitation gradient (Eq.~\ref{eq:full_model}).
...
To distinguish between these alternatives, we conducted a greenhouse experiment using \textit{A. hyemalis} and soils collected from each field site, crossing symbiont status ($S^+$/\revise{\linelabel{Sminus9}$S^-$}) with soil origin (seven sites; Fig.~\ref{fig:site}d; 
...
Herbivore access also increased realized herbivory: plants in herbivore-access plots experienced 24\% tiller damage on average, compared to 14\% in herbivore-exclusion plots (Fig.~\ref{sup:herb}; Fig.~\ref{sup:herbsp};Fig.~\ref{sup:herbivory}).
}
...
\revise{\linelabel{replantingresults}
Because plots at the two westernmost sites (KER and SON) were destroyed by deer and replanted in 2024, we repeated all analyses excluding these sites to confirm they did not drive our conclusions. 
This produced qualitatively similar patterns of symbiont effects across climate and herbivory treatments (Fig.~\ref{fig:sensitivity_KER_SON}, Table~\ref{tab:sensitivity_KER_SON}), indicating our results are robust to the inclusion of these replanted sites.}
