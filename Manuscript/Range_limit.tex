...
\tom{}{You need a paragraph introducing the natural history of the focal species.}
	
\subsection*{Source populations and symbiont removal}
\tom{}{Here I would describe the source populations and heat treatment details. We spent a lot of time look at how well the heat treatments actually caused endophyte elimination, versus working with natural E+ and E- that were not experimentally generated. That needs to be addressed here.}

\subsection*{Field experimental design}
We conducted our \tom{field experiment}{In the figure caption you describe the sites as common garden sites. If you want to use that language you should also include it here.} across \tom{seven sites}{You should create a table for the lat long of each site and a brief description of the site. This can go in a supplement. Lot/Long is important for meta-analysis, as you know.} spanning the precipitation gradient from Louisiana to central Texas, USA (Fig.~\ref{fig:site}). 
...
At each site, we established 24 1.5 m $\times$ 1.5 m plots, with each species assigned to separate plots (eight plots per species). 
\tom{In each plot we planted 15 individuals.  }{You will need to briefly describe the ``rules'' we used to assign plants to plots, regarding how many source populations and how many clonal replicates per plot.}
\tom{of one of three cool-season perennial grasses: \emph{Agrostis hyemalis}, \emph{Elymus virginicus}, and \emph{Poa autumnalis}.  
These species typically host seed-transmitted fungal endophytes \citep{shaw2011guide}.  }{This material should be presented in the study system section. You should also name each Epichloe species.}
\tom{Seeds were collected in summer–fall 2021 from naturally symbiotic populations and either heat-treated to remove symbionts ($E^-$) or left untreated ($E^+$), maintaining the same genetic lineages as the source populations.}{I would move this to the section above and significantly expand it. I see that you have more info in the supplement. I would move it here.}  
...
\tom{Full details of the study design are provided in the supplementary materials (Supplementary Method \ref{sec:study_design}).}{I will keep moving forward for now, but I suggest all of the field methods and experimental design information should be presented here and not in the supplement. This section will need some more work to ensure that the experimental design is presented in a complete and reproducible way. Have you looked at the field methods document on the google drive? That has a lot of valuable information.
\\
It is also striking to me that you have included very little information about the herbivore treatment (which included both fencing and insecticide), and no information about what actual herbivores feed on these plants.}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../Figure/clim_map.pdf}
...
Climate data in A–C are based on 30-year normals of climate predictions (1993–2023) from PRISM data (TOM:You'll need to cite prism.), which illustrate the precipitation gradient across sites (scale on the right is in mm (TOM:This should be shown in the figure)). 
Red dots indicate the locations of source populations, while grey dots represent Global Biodiversity Information Facility (GBIF) occurrence records for each species within the study area.(TOM:provide citation or url for GBIF) 
D) Cumulative precipitation measured during the study period (MONTH 2023 - MONTH 2025) at each site, ordered west to east (TOM: I would order the bars west to east. When you say ``measured'' is this actually PRISM data?).}
\label{fig:site}
\end{figure}
\tom{}{See TOM comments inside the figure legend. I cannot use the footnote function there.}

\subsection*{Demographic data collection}
\tom{We collected demographic data}{You have not said when the experiment was initiated} including survival, growth, and reproduction in \tom{May and June}{Be more specific. AGHY and POAU were censused in May, ELVI was censused later.} of 2023, 2024 and 2025, which coincided with the flowering season of \emph{Agrostis hyemalis}, \emph{Elymus virginicus}, and \emph{Poa autumnalis}. 
...
We recorded the number of inflorescences per plant and counted the number of spikelets on up to three inflorescences from reproducing plants.  
%Spikelet counts were limited to three reproducing tillers per plot due to the time-consuming nature of this measurement.  
\tom{We used the number of spikelets from these three tillers to estimate the average number of spikelets per plant.}{We did not count spikelets for AGHY. Also, is your analysis using the mean spikelets rather than raw spikelets? I think raw spikelets would be better, but probably not a big deal.}
\tom{}{You have not said anything about recording herbivore damage, which I think will need to be part of the analyses.}
\subsection*{Modeling the effects of aridity, herbivory  and symbiosis on grass demography}
To quantify how abiotic stress and \tom{herbivory}{I think we need to be more careful in referring to this as `herbivore exclusion' rather than `herbivory', since we manipulated the former but not directly the latter.} modify the demographic outcomes of plant–fungal interactions, we developed hierarchical models for survival, growth, and fertility. 
\tom{Survival was modeled with a Bernoulli distribution, growth with a Gaussian distribution, and fertility, measured as the number of inflorescences and, for \textit{E. virginicus} and \textit{P. autumnalis}, the number of spikelets, with negative binomial distributions.}{You need to say more about these response variables, and about how data from multiple years were used. For example, what is ``growth'', and how is this quantified? Are you including here the first few months of the experiment (planting to first census) or is the first transition years May 2023 - May 2024?}
\tom{All species were modeled jointly to estimate pooled fixed effects.}{This actually does not make sense to me. I have not looked at the code but I would think we want species-specific fixed effects, but pooled estimates for the random effect variance terms.}
All vital rates shared the same linear predictor for the expected value ($\mu$) for \tom{observation $i$}{You had ``individual'' here but that is not correct, because you include multiple years of data for each individual -- that's why it is important to clarify how multiple years were used.}:
\tom{}{I would either subscript the equation more thoroughly (observation i, species j, plot k, source l, site m) or not subscript it at all.}
...
\end{aligned}
\end{equation}
\noindent where $P$ is \tom{precipitation}{What does ``precipitation'' mean? How was this quantified? Over what time period? In what units? From what source? This needs a lot more detail.}, $endo$ indicates endophyte presence, and $herb$ indicates herbivore access. 
...
We included a second-order term for precipitation to accommodate the possibility that vital rates may peak at intermediate precipitation and decline at both low and high levels. 
...
\tom{}{I think the rationale for this model structure needs to be clearer. Why does endophyte interact with the second-order precipitation effect but herbivory does not? Why is there no endophyte-precip-herbivory three-way interaction?}
%Models were implemented in R \citep{RCoreTeam} using Stan via the RStan interface \citep{Rstan}.

Host species had independent fixed-effect parameters ($\beta$) for symbiosis, precipitation, and herbivore access, including their interactions, but shared variance terms for the following normally-distributed random effects: 
site-to-site heterogeneity not explained by precipitation ($\phi_{\text{site}[i]} \sim \mathcal{N}(0,\; \sigma_{\text{site}})$), source variation ($\omega_{\text{source}[i]} \sim \mathcal{N}(0,\; \sigma_{\text{source}})$) associated with the provenance of transplants used in the common garden, and plot-to-plot variation ($\rho_{\text{plot}[i]} \sim \mathcal{N}(0,\; \sigma_{\text{plot}})$). 
\tom{}{Question about the random effects. Did you you force species to share the deviates $\phi$, $\omega$, and $\rho$, or did they have unique deviates and only shared variances. Another way of asking that is, for example, is there a single pooled random effect for each site, or does each species get its own site random effect? I think we would want the latter, which would say that each species can do better or worse at different sites, even as they have the same distribution across sites.}
By modeling species jointly with this hierarchical structure, we ``borrow strength'' across species to better account for background nuisance variables.
