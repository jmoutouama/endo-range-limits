...
We  addressed two questions : (i) under combined abiotic  and biotic stress, do endophytes provide fitness benefits or impose costs on their host plants? (ii) which factors ( abiotic vs. biotic) influence whether the grass-endophyte interaction benefits or harms the host plant?
We predicted that endophytes will enhance host performance under low precipitation by mitigating drought stress (Fig.~\ref{fig:predcition}). 
These benefits are expected to be smaller in fenced plots with reduced herbivory, where the contribution of symbionts to host performance is less critical. 
...
Across fitness components, \textit{Epichloë} endophytes generally enhanced host performance under wetter conditions, while their effects were costly or occasionally neutral  under drier conditions. 
Benefits were not most apparent when herbivores were excluded.
...
\section*{Discussion}
Using a field experiment, Bayesian statistical modeling, and three cool-season perennial grasses infected with vertically transmitted Epichloë endophytes, we tested how this mutualism influences host demography along a natural climatic gradient in the United States.
Our results demonstrate that grass–endophytes symbioses generally enhance host performance under favorable, wetter conditions, whereas their effects can be neutral or even detrimental under high abiotic stress. 
...
These results underscore that the outcome of grass–\textit{Epichloë} symbiosis shifts along abiotic stress gradients, providing empirical support for environmentally contingent mutualism in a broadly distributed host–symbiont system.  

Endophyte presence often enhances host performance under favorable or moderately stressful conditions, a pattern widely documented, a pattern widely documented across grass–\textit{Epichloë} systems.
Endophytes can increase host growth, reproduction, and resistance to herbivores, but these benefits are most apparent when the host has sufficient resources, such as water and nutrients, to support the fungus. 
...
While endophyte infection enhanced host performance under favorable or moderately stressful conditions, its effects often diminished under high abiotic stress. 
In resource-limited environments, such as the driest sites in our study, the metabolic costs of maintaining the endophyte can outweigh the benefits, resulting in neutral or even negative effects on growth and survival.
This shift illustrates the context-dependent nature of grass–\textit{Epichloë} symbioses.
