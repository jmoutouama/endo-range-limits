...
% Fallback line numbers taken from the current compiled EcolLett_R1.tex.
% If EcolLett_R1.aux is found, xr uses the live values and these are ignored;
% if it is not found (e.g. Overleaf without a latexmkrc), these prevent `??'.
% Recompile EcolLett_R1.tex first whenever its text changes.
\makeatletter
\newcommand{\msfallback}[3]{\@ifundefined{r@ms-#1}{\global\@namedef{r@ms-#1}{{#2}{#3}}}{}}
\AtBeginDocument{\msfallback{Sminus1}{134}{6}}
\AtBeginDocument{\msfallback{Sminus2}{137}{6}}
\AtBeginDocument{\msfallback{Sminus3}{149}{6}}
\AtBeginDocument{\msfallback{Sminus4}{154}{7}}
\AtBeginDocument{\msfallback{Sminus5}{179}{7}}
\AtBeginDocument{\msfallback{Sminus7}{218}{8}}
\AtBeginDocument{\msfallback{alkaloid}{405}{14}}
\AtBeginDocument{\msfallback{beach}{140}{6}}
\AtBeginDocument{\msfallback{endocost}{102}{5}}
\AtBeginDocument{\msfallback{genotypedef}{144}{6}}
\AtBeginDocument{\msfallback{herbivoryresults}{278}{10}}
\AtBeginDocument{\msfallback{hostbenefits}{120}{6}}
\AtBeginDocument{\msfallback{inflorescence}{212}{8}}
\AtBeginDocument{\msfallback{nbind}{168}{7}}
\AtBeginDocument{\msfallback{ppcsoil}{268}{10}}
\AtBeginDocument{\msfallback{range-expansion}{418}{15}}
\AtBeginDocument{\msfallback{replanting}{183}{7}}
\AtBeginDocument{\msfallback{replantingresults}{328}{12}}
\AtBeginDocument{\msfallback{sghdiscussion}{348}{13}}
\AtBeginDocument{\msfallback{sghnuance}{44}{3}}
\AtBeginDocument{\msfallback{spikeletmethod}{195}{8}}
\AtBeginDocument{\msfallback{spikeletresults}{313}{12}}
\AtBeginDocument{\msfallback{stroma}{125}{6}}
\AtBeginDocument{\msfallback{symbiont-retention}{377}{13}}
\AtBeginDocument{\msfallback{taxofix}{119}{5}}
\AtBeginDocument{\msfallback{transmission}{375}{13}}
\AtBeginDocument{\msfallback{vital}{209}{8}}
\AtBeginDocument{\msfallback{whereas1}{198}{8}}
\makeatother
...
\response{We thank Reviewer 1 for highlighting this important biological implication. We agree that the stronger demographic benefits of endophytes under mesic conditions raise the possibility that the balance of costs and benefits could influence the persistence and distribution of the symbiosis across environmental gradients. We have expanded the Discussion to address this implication. Specifically, we now discuss the possibility that if endophyte-associated costs reduce host or symbiont fitness under dry conditions, endophyte prevalence could decline toward the dry end of environmental gradients, consistent with evidence that endophyte incidence varies substantially across broad environmental gradients \citep{afkhami2009endophyte, semmartin2015broad}. We emphasize, however, that this is a prediction arising from our demographic results rather than a pattern we directly tested, and that direct studies of endophyte prevalence and transmission across environmental gradients are needed to determine whether the demographic pattern we observed is associated with changes in symbiosis frequency (line \ref{ms-symbiont-retention}).}
...
\response{We thank Reviewer 1 for pointing out this important nuance. We agree that our original statement that vertically transmitted endophytes ``cannot be expelled'' was too strong. We have revised the text to clarify that, although \textit{Epichloë} endophytes are tightly integrated with their hosts and are not readily removed from established infected tissues, imperfect transmission can result in symbiont loss among tillers and across reproductive transitions. We now explicitly discuss how such transmission dynamics could influence the prevalence and persistence of the symbiosis when the costs of infection increase under stressful conditions (line \ref{ms-transmission}).\\
\\
Regarding the reviewer's question, we did not re-assess endophyte status at the end of the experiment and therefore cannot determine whether endophyte prevalence changed during the experiment or whether all tillers remained infected. Our analyses therefore assume that the initial symbiont treatment was maintained throughout the experiment. We now state this limitation explicitly and discuss how changes in endophyte prevalence or within-host distribution could affect the interpretation of the demographic responses (line \ref{ms-transmission}).}
...
\response{We thank Reviewer 1 for pointing out this inconsistency. We have replaced ``endo'' with ``symb'' in the model to consistently represent the $S^+$ versus $S^-$ treatment throughout the analysis.}
...
\response{We thank Reviewer 1 for catching this error. The raw percentages for \textit{A. hyemalis} were reported in an inconsistent order, which made it appear that 
$S^+$ plants experienced more herbivore damage. We now report model-based predictions from our binomial model of tiller damage (Fig.~S6) for all three species, consistently as $S^+$ vs.\ $S^-$. These show that endophyte symbiosis reduced damage in \textit{P. autumnalis} and, more weakly, in \textit{A. hyemalis}, but increased damage in \textit{E. virginicus} (line \ref{ms-herbivoryresults}).}

\comment{310: Cite the figure or table with the spikelet results.}
\response{We thank Reviewer 1 for catching this omission. We have added a citation to Table~S10, which reports the spikelet production model results for \textit{P. autumnalis} and \textit{E. virginicus}, at this point in the text (line \ref{ms-spikeletresults}).}
...
\response{We thank the reviewer for pointing this out. We have replaced the hyphen with a proper minus sign throughout the manuscript for designations such as $S^-$. We have also followed the journal's typographical convention of using an en dash for numerical ranges (lines \ref{ms-Sminus1}, \ref{ms-Sminus2}, \ref{ms-Sminus3}, \ref{ms-Sminus4}, \ref{ms-Sminus5},  \ref{ms-Sminus7}).}
...
\response{Thank you for the suggestion. We have changed  ``minutes'' with ``min'' as suggested.}
...
\response{We thank Reviewer 3 for pointing this out. We have revised the Methods to specify that seeds were surface-sterilized using a 10\% bleach solution \citep{Sahu2022sterilization} (line \ref{ms-beach}).}


\comment{Line 143: More detail needed here also: what temperature? What light regime?}
\response{Our greenhouses maintain a range of 65-85F under ambient light and humidity. We have added these details to the manuscript (line \ref{ms-beach}).}
...
\response{We thank the reviewer for pointing out this lack of clarity. We have revised the Methods to define ``genotype'' operationally as an individual derived from a single seed and its vegetatively propagated descendants, such that clones derived from the same genotype represent identical genetic backgrounds for both host and symbiont (line \ref{ms-genotypedef}). We now clarify that these source genotypes were maintained through vegetative propagation and refer readers to Table S1 for the number of source genotypes and clonal replicates represented in each source population. The relevance of these numbers is so that readers understand that the total replication of the experiment involves genetically identical clones, but to varying degrees for different species and populations. To make this point clearer, we have clarified how these vegetatively propagated plants relate to the plants included in the experiment (line \ref{ms-genotypedef}). 
...
\response{Thank you for pointing this out. We have clarified the numbers of $S^+$ and $S^-$ plants corresponding to each starting endophyte frequency (line \ref{ms-nbind}).}

\comment{Line 189: Should ``on up to three'' be ``on up of three''?}
\response{Thank you for the suggestion. We have rephrased this as ``on a maximum of three'' to clarify that spikelets were counted on no more than three inflorescences per reproducing individual (line \ref{ms-spikeletmethod}).}
...
\response{We thank the reviewer for the suggestion. We have removed this sentence to reduce redundancy and clarified in the preceding sentence that \textit{A. hyemalis} spikelets were too numerous and small to count (line \ref{ms-spikeletmethod}).}
...
\response{Thank you for pointing this out. We have clarified the terminology by specifying that fertility was quantified as inflorescence and spikelet production (line \ref{ms-vital}).}
...
\response{We agree that this distinction was not sufficiently clear. We have clarified that the hurdle model distinguishes between whether a plant produced any inflorescences and, conditional on producing any inflorescences, how many it produced (line \ref{ms-inflorescence}).}
...
\response{We thank Reviewer 3 for pointing out this inconsistency. We agree that the order of the values was confusing, and we have corrected it. All values are now model-based predictions from our tiller damage model (Fig.~S6), with each pair explicitly labelled as $S^+$ vs.\ $S^-$ (line \ref{ms-herbivoryresults}).}
...
\response{We thank the reviewer for this insightful suggestion. Although we did not directly test why previous studies have found host-range expansion effects of \textit{Epichloë}, we now speculate that these contrasting outcomes may depend on the environmental factors establishing host range margins. We have added this possibility to the Discussion, noting that symbiosis may promote host persistence or range expansion when endophyte benefits offset the factor limiting host performance, but provide little benefit or even constrain performance when the limiting factor is severe resource stress that the symbiosis cannot alleviate (line \ref{ms-range-expansion}).}
...
\response{We have corrected the color/order inconsistency between Figs.\ S2 and S3 (now S4 and S5) and matched color usage across Figs.\ 1, 5, and S10.}
